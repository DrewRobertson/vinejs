...
interface VineApi {
	...
	mountAll(configs: Array<{ target: VineTarget } & VineOptions>): void;
	destroyAll(): void;
}
...
const activeVines = new Set<VineInstance>();
const mountedTargets = new WeakMap<Element, VineInstance>();
 ...
 */
function getResponsiveBounds(width: number, swoop = 1) {
 ...
 */
function getSeamMetrics(container: Element, targetSection: Element | null, seamSelector: string, customSections: VineSections | null) {
 ...
 */
function getSegmentSpan(seg: VineSegment): number {
 ...
 */
function computeWrapperSegments(container: Element, targetSection: Element | null, seamSelector: string, customSections: VineSections | null, instanceSeed = 100, swoop = 1): VineSegment[] {
 ...
 */
function drawLeafBlade(p: any, x: number, y: number, rotation: number, leafLength: number, leafWidthRatio: number, maturityRatio: number, seed: number): void {
	p.randomSeed(seed);
	p.push();
	p.translate(x, y);
	p.rotate(rotation);
 ...
 */
function drawOrganicLeafWithStem(p: any, x: number, y: number, stemAngle: number, side: number, type: VineSegment['type'], segmentT: number, leafSeed: number): void {
	...
	const petiolePoints: VinePoint[] = [{ x, y }];
	let px = x;
	let py = y;
	...
	const tipAngle = p.atan2(
		p.bezierTangent(y, cpY, cpY, stemEndY, 1.0),
		p.bezierTangent(x, cpX, cpX, stemEndX, 1.0)
	);
	...
	if (isLongPetiole && p.random() < 0.25) {
		...
		const subTipAngle = p.atan2(
			p.bezierTangent(bPoint.y, bCpY, bCpY, bEndY, 1.0),
			p.bezierTangent(bPoint.x, bCpX, bCpX, bEndX, 1.0)
		);
 ...
 */
function drawVineSegment(p: any, seg: VineSegment, span: number, containerWidth: number, swoop = 1): void {
	const { x1, y1, cp1x, cp1y, cp2x, cp2y, x2, y2, type } = seg;
	p.noFill();

	const steps = 150;
	const coords: VineCoord[] = [];
 ...
 */
function createVineInstance(container: Element, options: VineOptions & {
		sections?: VineSections | null;
		targetSection?: Element | null;
	} = {}): VineInstance {
	const {
		seamSelector = '.site-section',
		sections = null,
		targetSection = null,
		layer = 'fg',
		seed = Math.floor(Math.random() * 1000000),
		filter = 'url(#earthy-paper-shader)',
		swoop = 1
	} = options;
 ...
 */
function resolveElements(target: VineTarget): Element[] {
	if (!target) return [];
 ...
 */
function createVine(target: VineTarget | (VineOptions & { target: VineTarget }), vineTypeOrSeedOrOptions: 'wrap' | number | VineOptions = 'wrap', options: VineOptions = {}): VineInstance | VineInstance[] {
	ensureVineStyles();
	ensureVineFilter();
	...
	if (typeof target === 'object' && target !== null && !('nodeType' in target) && !Array.isArray(target) && 'target' in target) {
		const config = target as VineOptions & { target: VineTarget };
		options = config;
	...
	const elements = resolveElements(target as VineTarget);
	if (elements.length === 0) return [];

	const instances: VineInstance[] = [];
	...
	elements.forEach(el => {
		...
		const vineElement = el as VineElement;
 ...
 */
function initVineSystem(configArray: Array<{ target: VineTarget } & VineOptions>): void {
